...
def display_image():
    ...
    while True:
        ...
        if frame is not None:
            ...
            # 进行检测
            # if flag:
            #     detector.init_detector(new_frame = frame,video_path=0, manual=False, load_bbox_from_file=False, target_match=False,
            #                   feature_match=True, save_bbox=False)
            #     flag = False
            # frame, kuang, center =detector.detect(frame)
...
def DecCBFun(nPort, pBuf, nSize, pFrameInfo, nUser, nReserved2):
    ...
    if pFrameInfo.contents.nType == 3:
        ...
        jpgName = 'test_stamp[%d].jpg'% pFrameInfo.contents.nStamp
        bmpName = 'test_stamp[%d].bmp'% pFrameInfo.contents.nStamp
        sFileName = os.path.join(project_dir,'pic',jpgName)
        ...
        test = Playctrldll.PlayM4_ConvertToBmpFile(pBuf, nSize, nWidth, nHeight, nType, c_char_p(sFileName.encode()))
        print(test)
        lRet = 0
